#!/usr/bin/env node
/**
 * Data Publisher with Domain Support for Observability Agent UI
 *
 * This script publishes sample data to NATS with JetStream domain support.
 *
 * Usage:
...
const config = {
  ...
  services: (args['services'] || 'payment-service,order-service,inventory-service,api-gateway').split(',').map(s => s.trim())
};
...
// Main function
async function main() {
  console.log(`Data Publisher with Domain Support for Observability Agent UI`);
  console.log(`----------------------------------------------`);
